...
class TarjetaColeccionGenerator(private val context: Context) {
    ...
    private fun drawHeader(canvas: Canvas, coleccion: Coleccion, items: List<Item>) {
        ...
        coleccion.imagenPath?.let { path ->
            loadAndCropBitmap(path, 110, 110)?.let { bmp ->
                ...
                val ringGradient = SweepGradient(
                    imgCx, imgCy,
                    intArrayOf(colorAccent1, colorAccent2, colorAccent3, colorAccent1),
                    null
                )
        ...
        canvas.drawRoundRect(
            ...
            3f,
            3f,
            Paint().apply { shader = accentGradient }
        )
    }
    ...
    private fun drawItemsGrid(canvas: Canvas, items: List<Item>) {
        ...
        items.forEachIndexed { index, item ->
            ...
            val accentBarGradient = LinearGradient(
                ...
                cy + cellH,
                accentColor,
                Color.parseColor("#00${Integer.toHexString(accentColor).substring(2)}"),
                Shader.TileMode.CLAMP
            )
            ...
            val imgLoaded = item.imagenPath?.let {
                loadAndCropBitmap(it, cellW.toInt(), imgH.toInt())
            }
            ...
            canvas.drawRoundRect(
                RectF(badgeX, badgeY, badgeX + badgeW, badgeY + 24f),
                12f,
                12f,
                Paint().apply { shader = badgeGradient; isAntiAlias = true }
            )
